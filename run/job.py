...
aida_obj = aida()
aida_obj.read_data(ctm_name, Path(ctm_dir), gas, ctm_freq, sensor+'_'+gas,
...
aida_obj.oi(error_ctm=error_ctm)
aida_obj.reporting(gas + '_' + str(year) + f"{month:02}", gas, output_pdf_dir)
aida_obj.write_to_nc(gas + '_' + str(year) + f"{month:02}", output_nc_dir)
